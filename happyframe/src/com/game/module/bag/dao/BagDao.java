...
public interface BagDao {
	...
	@SQL("update bag_item set itemId=:1.itemId, playerId =:1.playerId, count =:1.count where uid =:1.uid")
	public void updateBagItem(BagItem item);

	@SQL("insert into bag_item (uid, itemId, playerId, count) values(:1.uid, :1.itemId, :1.playerId, :1.count)")
	public int addBagItem(BagItem item);

	@SQL("SELECT * FROM bag_item where uid =:1")
	public BagItem getBagItem(int uId);

	@SQL("SELECT * FROM design_warehouse_lvup")
