...
public class EventJobFactory {
	...
	public static AbstractEventJob createExtention(String msg,
		...
		try {
			...
			newInstance.setChannel(channel);
			GameSession gameSsion = OnlinePlayerManager.getIntance().getGameSsionByChannel(channel);
			if(gameSsion.getPlayer() != null){
				newInstance.setPlayerId(gameSsion.getPlayer().getPlayerId());
			}
